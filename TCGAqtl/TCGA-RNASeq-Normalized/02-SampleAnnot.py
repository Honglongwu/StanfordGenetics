...
for s in Samples:
    ...
    D[s]['platform'] = sf[-1]
    if sf[3][0] == '0':
        D[s]['sample_type'] = 'tumor'
    elif sf[3][0] == '0':
        D[s]['sample_type'] = 'normal'
    else:
        D[s]['sample_type'] = 'control'
    #if D[s]['sample'] not in CL:
    #    print(s)
G = gender()
ouFile = open('TCGA_RNA-Seq_SampleAnnot', 'w')
ouFile.write('\t'.join(['sampleID', 'sample', 'cancer', 'platform','sample_type','gender','race','age','tumor_status','tissue']) + '\n')
for s in Samples:
    cl = []
    ...
    ouFile.write('\t'.join([s, D[s]['sample'], D[s]['cancer'], D[s]['platform'], D[s]['sample_type']]+cl) + '\n')
ouFile.close()
