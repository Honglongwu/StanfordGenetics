...
write_hdf5 = function(X, Y, myfile = "output.hdf5"){
...
X=data.frame(s1=c('AA','AB','BC'),s2=c('AC','BC','BB'),s3=c('NA','BB','AB'))
rownames(X) =c('chr1-102','chr2-203','chr3-304')
Y=data.frame(p1=c(1,2,2),s2=c(3,2,2),s3=c(1,1,2))
rownames(Y) = c('s1','s2','s3')

write_hdf5(X,Y,'test.hdf5')
